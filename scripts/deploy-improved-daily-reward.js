...
async function main() {
  console.log("Starting deployment of ImprovedDailyRewardChatToken...");

  // Get deployer address
  const [deployer] = await hre.ethers.getSigners();
  console.log("Deployer address:", deployer.address);

  // Deploy ImprovedDailyRewardChatToken
  const ImprovedDailyRewardChatToken = await hre.ethers.getContractFactory("ImprovedDailyRewardChatToken");
  const token = await ImprovedDailyRewardChatToken.deploy(deployer.address);
  await token.deployed();
  console.log("ImprovedDailyRewardChatToken deployed to:", token.address);
  ...
  console.log("\n=== Deployment Summary ===");
  console.log("ImprovedDailyRewardChatToken Address:", token.address);
  console.log("Deployer Address:", deployer.address);
